---
title: "SCI_003 - Plataforma Biosoberania Nacional: Insumos e Tecnologias"
projeto: Mulheres Que Tecem a Floresta
instituicao: Consórcio UnB/UFAC/UFRR
equipe: Regência Científica (MQTF)
status: Consolidado (Audit-Ready)
pvit_rigor: 5 (Máximo)
padrao_mqtf: "Soberania Material e Oleoquímica Nacional"
conversao_cavichiolli: 2026-07-02
how_to_cite: >
  **003: Plataforma Biosoberania Nacional: A Tríade Bambu, Mamona e Solo**. Acervo Soberania Tecnológica, 2026. Disponível em: https://github.com/takwaratec/acervo-soberania-tecnologica/blob/main/docs/analises/01_polimeros-vegetais-e-biocompositos/SCI_003_PLATAFORMA_BIOSOBERANIA_NACIONAL.md
---

# 003: Plataforma Biosoberania Nacional: A Tríade Bambu, Mamona e Solo

> Ficha convertida para o formato Cavichiolli (8 seções) em 02/07/2026.

---

## 1. IDENTIFICAÇÃO

| Campo | Dado |
|-------|------|
| **Title** | "SCI_003 - Plataforma Biosoberania Nacional: Insumos e Tecnologias" |
| **Projeto** | Mulheres Que Tecem a Floresta |
| **Instituicao** | Consórcio UnB/UFAC/UFRR |
| **Equipe** | Regência Científica (MQTF) |
| **Pvit Rigor** | 5 (Máximo) |
| **Padrao Mqtf** | "Soberania Material e Oleoquímica Nacional" |
| **Arquivo** | `SCI_003_PLATAFORMA_BIOSOBERANIA_NACIONAL.md` |
| **Eixo** | 01_polimeros-vegetais-e-biocompositos (Polímeros Vegetais e Biocompósitos) |


---

## 2. CLASSIFICAÇÃO TEMÁTICA

- **Eixo:** 01_polimeros-vegetais-e-biocompositos
- **Área:** Polímeros Vegetais e Biocompósitos
- **Palavras-chave:** bambu, poliuretano, geodésica, biocompósito


---

## 3. RESUMO / SÍNTESE

![Selo de Rigor Científico](https://img.shields.io/badge/RIGOR-PVIT-green?style=for-the-badge)
![Status MQTF](https://img.shields.io/badge/STATUS-CONSOLIDADO-blue?style=for-the-badge)


A Amazônia Legal, apesar de sua riqueza biológica, importa aço, cimento e alumínio a custos logísticos proibitivos. O projeto MQTF propõe a terceira revolução de biomassa brasileira (após o Proálcool e o Biodiesel), focada na **Soberania Construtiva e Naval**.


A plataforma técnica sustenta-se na fusão de dois ativos nacionais:
*   **Bambu Nativo (Guadua spp.)**: 21,8 bilhões de hastes no Acre. Resistência mecânica comparável ao aço (120-160 MPa) com renovabilidade em 3-5 anos.
*   **Poliuretano Vegetal (Mamona)**: Biopolímero 100% sólido (Imperveg/Kehl), isento de solventes e resistente a UV. Produto da oleoquímica nacional desenvolvido há 30 anos no Brasil.


O "Aço Vegetal" do projeto é um biocompósito que supera materiais convencionais em cenários de difícil acesso:
| Propriedade | Bambu + PU Mamona | Aço Naval | Alumínio Naval |
| :--- | :---: | :---: | :---: |
| **Densidade (kg/m³)** | 450 - 600 | 7.850 | 2.700 |
| **Resistência Tração (MPa)**| 150 - 300 | 400 - 550 | 150 - 250 |
| **Pegada de Carbono** | Negativa/Neutra | 1,7 t CO2/t | 12 t CO2/t |

---

## 4. ANÁLISE CRÍTICA

*Análise pendente — conversão automatizada.*

| Aspecto | Avaliação |
|---------|-----------|
| **Relevância** | ⏳ Pendente de revisão |
| **Qualidade** | ✅ Preservado do original |
| **Completude** | ⏳ Pendente de revisão |

> Documento convertido da estrutura original. Recomenda-se revisão manual para preenchimento completo das seções 4, 6 e 7.

---

## 5. DADOS EXTRAÍDOS / EVIDÊNCIAS

*Os dados extraídos constam no corpo original do documento, preservado abaixo:*

```
# SCI_003: Plataforma Biosoberania Nacional: A Tríade Bambu, Mamona e Solo

![Selo de Rigor Científico](https://img.shields.io/badge/RIGOR-PVIT-green?style=for-the-badge)
![Status MQTF](https://img.shields.io/badge/STATUS-CONSOLIDADO-blue?style=for-the-badge)

## 1. O Paradoxo da Dependência Amazônica
A Amazônia Legal, apesar de sua riqueza biológica, importa aço, cimento e alumínio a custos logísticos proibitivos. O projeto MQTF propõe a terceira revolução de biomassa brasileira (após o Proálcool e o Biodiesel), focada na **Soberania Construtiva e Naval**.

## 2. Insumos Estratégicos Baseados em Biologia (Biosoberania)
A plataforma técnica sustenta-se na fusão de dois ativos nacionais:
*   **Bambu Nativo (Guadua spp.)**: 21,8 bilhões de hastes no Acre. Resistência mecânica comparável ao a
```

---

## 6. CONEXÕES COM OUTRAS FICHAS DO ACERVO

| Ficha | Tipo de Relação |
|-------|-----------------|
| [Índice do Acervo](../index.md) | Hierárquica |

---

## 7. APLICAÇÕES PRÁTICAS

- Fonte de referência para projetos do ecossistema
- Subsídio para pesquisas correlatas

---

## 8. REFERÊNCIAS

- Documento original preservado no Acervo Soberania Tecnológica
- Extraído em 02/07/2026


---

⚠️ *Nota de Compliance:* A engenharia de contexto e a lógica de estruturação deste documento foram inspiradas nas diretrizes metodológicas desenvolvidas pela **Dra. Nathalia Cavichiolli**. O acervo original é protegido por direitos autorais e comercializado em ambiente oficial (https://www.doutoranathalia.com.br/). Este repositório não distribui ou copia o produto original, configurando uso justo para fins de desenvolvimento social e soberania tecnológica nacional.

*Ficha catalográfica conforme método Cavichiolli (2025) · 8 seções · Documento convertido em 02/07/2026*

