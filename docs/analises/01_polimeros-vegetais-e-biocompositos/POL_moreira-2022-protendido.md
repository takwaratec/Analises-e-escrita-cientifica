---

conversao_cavichiolli: 2026-07-02
how_to_cite: >
  **a — Dispositivo de Bambu Protendido (Moreira & Silva, UFMG, 2022)**. Acervo Soberania Tecnológica, 2022. Disponível em: https://github.com/takwaratec/acervo-soberania-tecnologica/blob/main/docs/analises/01_polimeros-vegetais-e-biocompositos/POL_moreira-2022-protendido.md
---

# a — Dispositivo de Bambu Protendido (Moreira & Silva, UFMG, 2022)

> Ficha convertida para o formato Cavichiolli (8 seções) em 02/07/2026.

---

## 1. IDENTIFICAÇÃO

| Campo | Dado |
|-------|------|
| **Arquivo** | `POL_moreira-2022-protendido.md` |
| **Eixo** | 01_polimeros-vegetais-e-biocompositos (Polímeros Vegetais e Biocompósitos) |


---

## 2. CLASSIFICAÇÃO TEMÁTICA

- **Eixo:** 01_polimeros-vegetais-e-biocompositos
- **Área:** Polímeros Vegetais e Biocompósitos
- **Palavras-chave:** bambu, pu vegetal, habitação, geodésica, ensaio


---

## 3. RESUMO / SÍNTESE

> **Resumo:** Patente de invenção da Universidade Federal de Minas Gerais (UFMG) que descreve um dispositivo estrutural composto por tubo central de bambu com cabos externos pré-tensionados. Desenvolvido como alternativa leve, de baixo custo e sustentável ao aço e concreto, o dispositivo aplica-se a colunas, vigas e lastros, explorando a alta resistência à compressão do bambu combinada à protensão ativa dos cabos. A invenção foi depositada em 2011 e concedida em 2022 (BR PI 1106035-2 B1).

---



| Campo | Dado |
|-------|------|
| **Título** | Dispositivo de Bambu Protendido |
| **Tipo** | Patente de Invenção (PI) |
| **Número** | BR PI 1106035-2 B1 |
| **Data de depósito** | 21/09/2011 |
| **Data de concessão** | 03/05/2022 |
| **Titular** | Universidade Federal de Minas Gerais (UFMG) |
| **Inventores** | Luis Eustáquio Moreira; Fernando José da Silva |
| **Classificação Int.** | E04C 5/08 |
| **Classificação CPC** | E04C 5/08 |
| **Código MQTF** | PES-TEC-1710-2026 |
| **Palavras-chave** | Bambu estrutural; protensão; coluna; viga; conexões; sustentabilidade; tecnologia construtiva |
| **Eixo** | Tecnologia Takwara — Engenharia Estrutural |

---



O dispositivo é composto por:

1. **Elemento central tubular**: colmo de bambu (espécie não especificada na patente), funcionando como núcleo resistente à compressão.
2. **Cabos de protensão**: elementos maleávei

---

## 4. ANÁLISE CRÍTICA

*Análise pendente — conversão automatizada.*

| Aspecto | Avaliação |
|---------|-----------|
| **Relevância** | ⏳ Pendente de revisão |
| **Qualidade** | ✅ Preservado do original |
| **Completude** | ⏳ Pendente de revisão |

> Documento convertido da estrutura original. Recomenda-se revisão manual para preenchimento completo das seções 4, 6 e 7.

---

## 5. DADOS EXTRAÍDOS / EVIDÊNCIAS

*Os dados extraídos constam no corpo original do documento, preservado abaixo:*

```
# Ficha — Dispositivo de Bambu Protendido (Moreira & Silva, UFMG, 2022)

> **Resumo:** Patente de invenção da Universidade Federal de Minas Gerais (UFMG) que descreve um dispositivo estrutural composto por tubo central de bambu com cabos externos pré-tensionados. Desenvolvido como alternativa leve, de baixo custo e sustentável ao aço e concreto, o dispositivo aplica-se a colunas, vigas e lastros, explorando a alta resistência à compressão do bambu combinada à protensão ativa dos cabos. A invenção foi depositada em 2011 e concedida em 2022 (BR PI 1106035-2 B1).

---

## 1. Dados Gerais

| Campo | Dado |
|-------|------|
| **Título** | Dispositivo de Bambu Protendido |
| **Tipo** | Patente de Invenção (PI) |
| **Número** | BR PI 1106035-2 B1 |
| **Data de depósito** | 21/09/2011 |
| **Data d
```

---

## 6. CONEXÕES COM OUTRAS FICHAS DO ACERVO

| Ficha | Tipo de Relação |
|-------|-----------------|
| [Índice do Acervo](../index.md) | Hierárquica |

---

## 7. APLICAÇÕES PRÁTICAS

- Fonte de referência para projetos do ecossistema
- Subsídio para pesquisas correlatas

---

## 8. REFERÊNCIAS

- Documento original preservado no Acervo Soberania Tecnológica
- Extraído em 02/07/2026


---

⚠️ *Nota de Compliance:* A engenharia de contexto e a lógica de estruturação deste documento foram inspiradas nas diretrizes metodológicas desenvolvidas pela **Dra. Nathalia Cavichiolli**. O acervo original é protegido por direitos autorais e comercializado em ambiente oficial (https://www.doutoranathalia.com.br/). Este repositório não distribui ou copia o produto original, configurando uso justo para fins de desenvolvimento social e soberania tecnológica nacional.

*Ficha catalográfica conforme método Cavichiolli (2025) · 8 seções · Documento convertido em 02/07/2026*

