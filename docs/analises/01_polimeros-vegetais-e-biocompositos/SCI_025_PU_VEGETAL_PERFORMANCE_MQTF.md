---
title: "SCI_025 - PU Vegetal: Performance, Aceitação e Mercado (Brasil-UE)"
projeto: Mulheres Que Tecem a Floresta
instituicao: Consórcio UnB/UFAC/UFRR
equipe: Regência Científica (MQTF)
status: Consolidado (Audit-Ready)
pvit_rigor: 5 (Máximo)
padrao_mqtf: "Não Veneno, Não Cimento, Não Voláteis"
conversao_cavichiolli: 2026-07-02
how_to_cite: >
  **025: Análise Estratégica da Imperveg Polímeros Vegetais**. Acervo Soberania Tecnológica, 2026. Disponível em: https://github.com/takwaratec/acervo-soberania-tecnologica/blob/main/docs/analises/01_polimeros-vegetais-e-biocompositos/SCI_025_PU_VEGETAL_PERFORMANCE_MQTF.md
---

# 025: Análise Estratégica da Imperveg Polímeros Vegetais

> Ficha convertida para o formato Cavichiolli (8 seções) em 02/07/2026.

---

## 1. IDENTIFICAÇÃO

| Campo | Dado |
|-------|------|
| **Title** | "SCI_025 - PU Vegetal: Performance, Aceitação e Mercado (Brasil-UE)" |
| **Projeto** | Mulheres Que Tecem a Floresta |
| **Instituicao** | Consórcio UnB/UFAC/UFRR |
| **Equipe** | Regência Científica (MQTF) |
| **Pvit Rigor** | 5 (Máximo) |
| **Padrao Mqtf** | "Não Veneno, Não Cimento, Não Voláteis" |
| **Arquivo** | `SCI_025_PU_VEGETAL_PERFORMANCE_MQTF.md` |
| **Eixo** | 01_polimeros-vegetais-e-biocompositos (Polímeros Vegetais e Biocompósitos) |


---

## 2. CLASSIFICAÇÃO TEMÁTICA

- **Eixo:** 01_polimeros-vegetais-e-biocompositos
- **Área:** Polímeros Vegetais e Biocompósitos
- **Palavras-chave:** bambu, pu vegetal, poliuretano, geodésica, resíduo, governança, norma


---

## 3. RESUMO / SÍNTESE

![Selo de Rigor Científico](https://img.shields.io/badge/RIGOR-PVIT-green?style=for-the-badge)
![Status MQTF](https://img.shields.io/badge/STATUS-CONSOLIDADO-blue?style=for-the-badge)


A Imperveg Polímeros Vegetais (fundada em 2008) representa uma vanguarda tecnológica no desenvolvimento de poliuretano vegetal derivado do óleo de rícino (*Ricinus Communis*). Esta tecnologia é um pilar do sistema **MQTF (Mulheres Que Tecem a Floresta)**, pois elimina a dependência de resinas de base fóssil e solventes tóxicos na construção civil e saneamento.


Ao contrário das resinas tradicionais de poliuretano (PUR) e Epóxi, a solução Imperveg é **100% sólida e isenta de solventes**.



| Característica Técnica | Poliuretano Vegetal (Imperveg) | Resinas Epóxi/PUR Fósseis |
| :--- | :--- | :--- |
| **Origem da Matriz** | Óleo de Mamona (Fonte Renovável) | Petróleo (Fonte Não Renovável) |
| **Conteúdo de Sólidos** | 100% (Isento de Solventes) | Frequentemente < 70% |
| **Toxicidade** | Atóxica (Atende NBR 12170) | Alta (Libera VOCs e metais pesados) |
| **Sustentabilidade** | Biodegradável / Baixa pegada CO2 | Persistente no ambiente |
| **Processamento** | Mistura a frio | Requer catalisadores voláteis |


*   **RQI 132 MR**: C

---

## 4. ANÁLISE CRÍTICA

*Análise pendente — conversão automatizada.*

| Aspecto | Avaliação |
|---------|-----------|
| **Relevância** | ⏳ Pendente de revisão |
| **Qualidade** | ✅ Preservado do original |
| **Completude** | ⏳ Pendente de revisão |

> Documento convertido da estrutura original. Recomenda-se revisão manual para preenchimento completo das seções 4, 6 e 7.

---

## 5. DADOS EXTRAÍDOS / EVIDÊNCIAS

*Os dados extraídos constam no corpo original do documento, preservado abaixo:*

```
# SCI_025: Análise Estratégica da Imperveg Polímeros Vegetais

![Selo de Rigor Científico](https://img.shields.io/badge/RIGOR-PVIT-green?style=for-the-badge)
![Status MQTF](https://img.shields.io/badge/STATUS-CONSOLIDADO-blue?style=for-the-badge)

## 1. Visão Geral (Sovereign Industrial Logic)
A Imperveg Polímeros Vegetais (fundada em 2008) representa uma vanguarda tecnológica no desenvolvimento de poliuretano vegetal derivado do óleo de rícino (*Ricinus Communis*). Esta tecnologia é um pilar do sistema **MQTF (Mulheres Que Tecem a Floresta)**, pois elimina a dependência de resinas de base fóssil e solventes tóxicos na construção civil e saneamento.

## 2. Diferenciais Técnicos e Vantagens Ambientais
Ao contrário das resinas tradicionais de poliuretano (PUR) e Epóxi, a solução Imper
```

---

## 6. CONEXÕES COM OUTRAS FICHAS DO ACERVO

| Ficha | Tipo de Relação |
|-------|-----------------|
| [Índice do Acervo](../index.md) | Hierárquica |

---

## 7. APLICAÇÕES PRÁTICAS

- Fonte de referência para projetos do ecossistema
- Subsídio para pesquisas correlatas

---

## 8. REFERÊNCIAS

- Documento original preservado no Acervo Soberania Tecnológica
- Extraído em 02/07/2026


---

⚠️ *Nota de Compliance:* A engenharia de contexto e a lógica de estruturação deste documento foram inspiradas nas diretrizes metodológicas desenvolvidas pela **Dra. Nathalia Cavichiolli**. O acervo original é protegido por direitos autorais e comercializado em ambiente oficial (https://www.doutoranathalia.com.br/). Este repositório não distribui ou copia o produto original, configurando uso justo para fins de desenvolvimento social e soberania tecnológica nacional.

*Ficha catalográfica conforme método Cavichiolli (2025) · 8 seções · Documento convertido em 02/07/2026*

